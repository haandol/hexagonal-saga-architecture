...
func (s *HotelService) Book(ctx context.Context, cmd *command.BookHotel) error {
	...
	if err != nil {
		...
		go func(reason string) {
			if err := s.hotelProducer.PublishAbortSaga(ctx, cmd.CorrelationID, cmd.ParentID, cmd.Body.TripID, reason); err != nil {
	...
	if err := s.hotelProducer.PublishHotelBooked(ctx, cmd.CorrelationID, cmd.ParentID, booking); err != nil {
		return err
	}
...
func (s *HotelService) CancelBooking(ctx context.Context, cmd *command.CancelHotelBooking) error {
	...
	if err := s.hotelProducer.PublishHotelBookingCancelled(ctx, cmd.CorrelationID, cmd.ParentID, booking); err != nil {
		return err
	}
